...
M-estimation suggests that $\hat{\theta}^{\text{ve}} = \arg\max_{\theta} \bar{J}_{n}(\theta)$ should converge to $\bar{\theta} = \arg\max_{\theta} \bar{J}(\theta)$ where $\bar{J}(\theta) = \mathbb{E}_{\theta^\star}[\bar{J}_\mathbf{Y}(\theta)] =  \mathbb{E}_{\theta^\star}[J_\mathbf{Y}(\theta, \hat{\psi}(\theta, \mathbf{Y}))]$. 
...
If $\hat{\theta}$ is ht MLE, then the Fisher Information matrix
...
Derive confidences interval for the inverse covariance $\mathbf{\Omega}$ and the regression parameters $\mathbf{B}$.

---

# Variance: naïve approach

Do as if $\hat{\theta}^{\text{ve}}$ was a MLE and $J_n$ the log-likelihood.
...
$\hat{\mathbb{V}}(\mathbf{B}_{kj}) = [n (\mathbf{X}^\top \mathrm{diag}(\mathrm{vec}(\hat{A}_{.j})) \mathbf{X})^{-1}]_{kk}, \qquad \hat{\mathbb{V}}(\Omega_{kl}) = 2\hat{\Omega}_{kk}\hat{\Omega}_{ll}$

The confidence intervals at level $\alpha$ are given by 

$B_{kj} =  \hat{B}_{kj} \pm \frac{q_{1 - \alpha/2}}{\sqrt{n}} \sqrt{\hat{\mathbb{V}}(\mathbf{B}_{kj})}, \qquad \Omega_{kl} =  \hat{\Omega}_{kl} \pm \frac{q_{1 - \alpha/2}}{\sqrt{n}} \sqrt{\hat{\mathbb{V}}(\Omega_{kl})}$. 
...
- For $\theta = (\mathbf{B}, \boldsymbol\Omega)$, $\hat{C}_n$ requires the inversion of $n$ matrices with $(p^2+pd)$ rows/columns\dots...
...
\end{aligned}
$$
$\rightsquigarrow$ Pratically not very useful since $\Sigma$ is unknown

#### Ongoing work 

Derive the formula with unknown $\boldsymbol\Sigma$
- Plugin-in  $\hat{\Sigma}$ in the formula of $\hat{C}_n$ leads very poor results\dots
- Need to account for cross-terms in $\nabla_{\theta \psi_i} \bar{J}_i(\theta; \hat{\psi}_i)$ between $\Omega$ and $\psi_i$...
- Use Jackknife resampling to estimate the variance\dots
