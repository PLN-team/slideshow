...
```{r covariance-PCA}
pheatmap(cov2cor(sigma(NR_PLN_Stream_PC)))
pheatmap(cov2cor(sigma(NR_PLNPCA)))
pheatmap(cov2cor(sigma(NR_PLNPCA_Stream)))
```
...
```{r ZIPLNNET}
NR_ZIPLNNET_all <- ZIPLNnetwork(Abundance ~ 0 + stream + PC1 | 0 + stream,  data = NR_DATA, control = ZIPLNnetwork_param(penalize_diagonal = FALSE))
...
fit_ZIPLN <- as.data.frame(NR_ZIPLNNET$fitted) %>% 
  setNames(colnames(NR_DATA$Abundance))
latent_ZIPLN <- as.data.frame(NR_ZIPLNNET$latent) %>% 
  setNames(colnames(NR_DATA$Abundance))
latent_pos_ZIPLN <- as.data.frame(NR_ZIPLNNET$latent_pos) %>% 
  setNames(colnames(NR_DATA$Abundance))
B_ZIPLN <- as.data.frame(NR_ZIPLNNET$model_par$B) %>% 
  setNames(colnames(NR_DATA$Abundance))
B0_ZIPLN <- as.data.frame(NR_ZIPLNNET$model_par$B0) %>% 
  setNames(colnames(NR_DATA$Abundance))
prob_zi <- as.data.frame(NR_ZIPLNNET$var_par$R) %>% 
  setNames(colnames(NR_DATA$Abundance))
```
...
pheatmap(prob_zi, annotation_row = site, show_rownames = FALSE)
```

```{r B0}
pheatmap(B0_ZIPLN, show_rownames = FALSE)
```
