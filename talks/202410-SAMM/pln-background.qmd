...
- MCMC techniques on reduced sampling space: composite likelihood [@pln-composite], importance sampling in PLN-PCA [@plnpca-importance]
...
## Empirical consistency of $\mathbf{B}$

$B^{\star}_{kj} \sim \mathcal N(2,1)$, $\Sigma$ Toeplitz, $x_i \sim \mathcal{M}(1, \pi = (1/d,..,1/d))$ with $k$ groups.

![Root mean square error](figs/sandwich-rmse.png)

## Empirical coverage

$\hat{\mathbb{V}}(\hat{\mathbf{B}}_{v})^{- \frac 1 2}  \left(   \hat{\mathbf{B}}_{v} -  \mathbf{B}^{\star}_{v} \right)$ for vectorized $\mathbf{B}_v$

![Coverage](figs/sandwich-coverage.png)
