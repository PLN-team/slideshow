...
# Variational Inference {background-color="#40666e"}
...
with Gaussian and Bernoulli distribution for $Z_{ij}$ and $W_{ij}$, then
...
**Property**: $J$ is separately concave in each element of $\theta, \psi$.
...
$$p(\mathbf{Z}_i, \mathbf{W}_i | \mathbf{Y}_i) = 
\left\{\begin{array}{l}
p(\mathbf{W}_i | \mathbf{Z}_i , \mathbf{Y}_i) p(\mathbf{Z}_i | \mathbf{Y}_i) \\[1ex]
p(\mathbf{Z}_i | \mathbf{W}_i , \mathbf{Y}_i) p(\mathbf{W}_i | \mathbf{Y}_i) \\
\end{array}\right.$$
...
$$\varphi(\mu, \sigma^2)\approx \tilde \varphi(\mu, \sigma^2)=  \frac{\exp \left(-\frac{W^2\left(\sigma^2 e^\mu\right)+2 W\left(\sigma^2 e^\mu\right)}{2
\sigma^2}\right)}{\sqrt{1+W\left(\sigma^2 e^\mu\right)}},$$
where $W(\cdot)$ is the Lambert function (i.e. $z = x \exp(x) \Leftrightarrow x = W(z), x,z \in \mathbb R$).
