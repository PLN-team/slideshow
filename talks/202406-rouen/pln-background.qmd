...
$$
\log p_{\theta}(\mathbf{X}_i) \geq \mathbb{E}_{q_{\psi}(\mathbf{Z}_i|\mathbf{X}_i)}\left[\log
p_{\theta}(\mathbf{X}_i | \mathbf{Z}_i)\right] - D_{KL}(q_{\psi}(\mathbf{Z}_i|\mathbf{X}_i)||p_{\theta}(\mathbf{Z}_i))
$$
