...
\begin{frame}{Choice of the variational family}
    \begin{itemize}
    \item \textbf{\textcolor{red}{Standard}} variational approximation; assume independance between $\boldsymbol{Z|Y}$ and $\boldsymbol{W|Y}$:
    \[ \scalebox{1.2}{$
...
q_{\psi_i}(W_{ij}) = \mathbf{\colorboxed{red}{\otimes_{j=1}^p \mathcal N\left( M_{ij}, S_{ij}^2\right) \mathcal B\left(P_{ij} \right)}}.$}\]

    \item \textbf{\textcolor{blue}{Enhanced}} variational approximation:
    model the conditional dependency by using the decomposition

        \[\scalebox{1.1}{$ Z_{ij}|
    W_{ij}, Y_{ij} = \left(Z_{ij}|Y_{ij}, W_{ij} = 1 \right)^{ W_{ij}}\left(Z_{ij}|Y_{ij}, W_{ij} = 0 \right)^{1-
W_{ij}}.$}\]
gives the following variational approximation:
...
\textbf{Bi-concavity} holds for the standard variational approximation.
    \end{itemize}
\end{frame}
\end{document}
